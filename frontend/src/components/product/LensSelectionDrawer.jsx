...
export function LensSelectionDrawer({
  ...
  const compatiblePackages = useMemo(
    () => availablePackages.filter((item) => isAllowedForPowerType(item, selectedPower?.type)),
  ...
  const noPackageRequired = selectedPower?.type === 'frame-only' && compatiblePackages.length === 0;
  ...
  useEffect(() => {
    ...
    const matchingPackages = availablePackages.filter((item) => isAllowedForPowerType(item, initialPower?.type));
  ...
  const selectPowerType = (type) => {
    const matchingPackages = availablePackages.filter((item) => isAllowedForPowerType(item, type));
    setPowerType(type);
    setActiveTag('all');
