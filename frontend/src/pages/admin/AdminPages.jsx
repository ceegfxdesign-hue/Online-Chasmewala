...
import {
  ...
  useGetAdminReturnsQuery, useGetAdminReviewsQuery, useGetAdminUsersQuery, useGetCategorySplitQuery, useGetDashboardQuery,
  ...
  useSetUserActiveMutation, useSetUserRoleMutation, useUpdateAdminBannerMutation, useUpdateAdminCouponMutation,
  useUpdateAdminReturnMutation, useUpdateBrandMutation, useUpdateCategoryMutation, useUpdateSettingsMutation,
} from '@/features/admin/adminApi';

const colors = ['#087F7B', '#0B1428', '#C9A45C', '#526174', '#D95C55'];
...
const toTrustBenefitDraft = (item) => ({
  key: `${item.title}-${item.subtitle}-${Math.random().toString(36).slice(2)}`,
  title: item.title || '',
...
export function SettingsPage() {
  ...
  const [trustBenefitItems, setTrustBenefitItems] = useState(() =>
    DEFAULT_TRUST_BENEFITS.map(toTrustBenefitDraft)
  );
  ...
  useEffect(() => {
    if (!Array.isArray(data?.trustBenefits) || !data.trustBenefits.length) return;
    setTrustBenefitItems(data.trustBenefits.map(toTrustBenefitDraft));
  }, [data?.trustBenefits]);
  ...
  const submit = async (event) => {
    ...
    body.trustBenefits = trustBenefitItems.map(({ title: benefitTitle, subtitle }) => ({
      ...
      <Card>
        <CardBody>
          <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2 mt-3 border-t border-navy-100 pt-5">
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-navy-500">Edit, add, or remove benefits shown on the homepage. Visitors can scroll sideways when needed.</p>
                </div>
                <Button
                  type="button"
                  variant="secondary"
                  size="sm"
                  leftIcon={<FiPlus />}
                  disabled={trustBenefitItems.length >= 12}
                  onClick={() => setTrustBenefitItems((items) => [
                    ...items,
                    toTrustBenefitDraft({ title: '', subtitle: '' }),
                  ])}
                >
                  Add benefit
                </Button>
            ...
            {trustBenefitItems.map((item, index) => (
              <div
                key={item.key}
                className="grid gap-4 rounded-2xl border border-navy-100 bg-surface-subtle p-4 md:col-span-2 md:grid-cols-[1fr_1fr_auto]"
              >
                ...
                <Input
                  ...
                  required
                  maxLength={80}
                  onChange={(event) => setTrustBenefitItems((items) => items.map((entry, itemIndex) => (
                ...
                <Input
                  ...
                  maxLength={160}
                  onChange={(event) => setTrustBenefitItems((items) => items.map((entry, itemIndex) => (
                    itemIndex === index ? { ...entry, subtitle: event.target.value } : entry
                  )))}
                />
                <Button
                  type="button"
                  ...
                  disabled={trustBenefitItems.length === 1}
                  onClick={() => setTrustBenefitItems((items) => items.filter((_, itemIndex) => itemIndex !== index))}
                >
                  <FiTrash2 />
                </Button>
              </div>
